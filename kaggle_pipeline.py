...
def _score_set(estimator, X, y, classes):
...
def evaluate(estimator, X_train, y_train, X_test, y_test) -> dict:
    ...
    t0 = time.perf_counter()
    auc, acc, gm, ce = _score_set(estimator, X_test, y_test, classes)
    predict_time = time.perf_counter() - t0

    tr_auc, tr_acc, tr_gm, tr_ce = _score_set(estimator, X_train, y_train, classes)
