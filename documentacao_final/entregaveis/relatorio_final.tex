...
Para complementar a inferência frequentista de Nemenyi, conduzimos o \textit{Bayesian Signed-Rank Test} proposto por Benavoli et al. (2017), valendo-nos da biblioteca \textit{baycomp}. Foram elaboradas duas comparações estritas: a primeira contra a melhor variante de GBDT ajustada (LightGBM Tuned), e a segunda contra o estado-da-arte em AutoML (AutoGluon Extreme 4h). Estipulou-se uma Região de Equivalência Prática (ROPE) estrita de 0.01 de diferença marginal em AUC para ambas.
...
O primeiro teste (Figura \ref{fig:bayesian_lgb}) infere que existe uma probabilidade massiva (superior a $83\%$) de que o TabICL e o LightGBM Tuned sejam estritamente equivalentes em aplicações do mundo real (diferença abarcada pela ROPE de $1\%$), com uma preferência marginal em favor do TabICL ($16.8\%$ de probabilidade de vitória estrita). 

O segundo teste (Figura \ref{fig:bayesian_ag}), considerado o confronto definitivo deste trabalho, demonstra uma probabilidade colossal de $99.6\%$ de que o TabICL é estatisticamente equivalente ao AutoGluon Extreme 4h. Isso consolida o fato de que um modelo de \textit{In-Context Learning} puro consegue reproduzir a fronteira de decisão de um complexo *ensemble* de AutoML sem nenhuma etapa de treinamento.
