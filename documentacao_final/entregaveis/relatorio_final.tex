...
Para complementar a inferência frequentista de Nemenyi, conduzimos o \textit{Bayesian Signed-Rank Test} proposto por Benavoli et al. (2017), valendo-nos da biblioteca \textit{baycomp}. A comparação foi realizada entre o TabICL e o LightGBM Tuned para a métrica AUC-OVO. Estipulou-se uma Região de Equivalência Prática (ROPE) estrita de 0.01 de diferença marginal em AUC.
...
O teste indica (Figura \ref{fig:bayesian_simplex}) que a probabilidade de equivalência prática entre os modelos, dentro da ROPE de $1\%$, é superior a $82\%$, com probabilidade de vitória estrita em favor do TabICL da ordem de $17\%$, e probabilidade residual de vitória do LightGBM Tuned inferior a $1\%$.
...
Todos os códigos estão organizados com o gerenciador de pacotes \texttt{uv}, com dependências fixadas em \texttt{pyproject.toml} e \texttt{uv.lock}, permitindo reprodução do ambiente em qualquer máquina com suporte a PyTorch.
